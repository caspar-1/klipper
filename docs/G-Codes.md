...
- `SAVE_GCODE_STATE [NAME=<state_name>] [RESET=1]`: Save the current
  ...
  not provided it defaults to "default". If RESET=1 is provided then
  after saving the state, the current state is reset to the startup
  default along with a zero relative extruder "E" position (G92 E0).
